...
function getCartTotalAmount() {
  return SHOPPING_CART.reduce((sum, item) => {
...
function addProductToCart(productId) {
  // increment existing item quantity or add new item with quantity 1
  const targetProduct = SHOPPING_CART.findIndex((item) => item.id === productId);
  if (targetProduct !== -1) {
    SHOPPING_CART[targetProduct].quantity += 1;
  } else {
...
function removeProductFromCart(productId) {
  // decrement quantity if >1, otherwise remove the item
  const targetProduct = SHOPPING_CART.findIndex((item) => item.id === productId);
  if (targetProduct === -1) return;

  if (SHOPPING_CART[targetProduct].quantity > 1) {
    SHOPPING_CART[targetProduct].quantity -= 1;
  } else {
    SHOPPING_CART.splice(targetProduct, 1);
  }
}
